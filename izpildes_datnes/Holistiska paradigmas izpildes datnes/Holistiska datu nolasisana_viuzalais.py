...
FROM synthea_kludainie_dati1_laboti
...
if df.empty:
    print("Nav atrastu datu kvalitātes problēmu rezultātu tabulā `synthea_kludainie_dati1_laboti`.")
else:
    ...
    # 1. Alternatīva - horizontālu uzkrāto stabiņu diagramma ar kvalitātes sliekšņiem 
    ...
    plt.title('Laboto datu (Holistiskā) kvalitātes novērtējums pa tabulām un kvalitātes dimensijām', pad=20, fontsize=14)
    ...
    # Sakārto tabulas un dimensijas pēc kļūdu īpatsvara no lielākās uz mazāko
    pivot_table = pivot_table.loc[
        pivot_table.mean(axis=1).sort_values(ascending=False).index, # Rindas (tabulas)
        pivot_table.mean().sort_values(ascending=False).index # Kolonnas (kļūdu veidi)
    ]
    
    # Zīmē siltuma karti:
    plt.imshow(
        pivot_table.fillna(0), # Aizvieto trūkstošās vērtības ar 0
        cmap='OrRd', # Krāsu shēma (oranža-sarkana)
        aspect='auto' # Automātisks mērogojums
    )
    plt.colorbar(label='Kļūdu īpatsvars (%)') #Pievieno krāsu leģendu ar % skalu
    
    # Ievieto vērtības šūnās
    ...
    plt.title('\n Laboto datu (Holistiskā) kvalitātes siltuma karte', pad=20, fontsize=14)
