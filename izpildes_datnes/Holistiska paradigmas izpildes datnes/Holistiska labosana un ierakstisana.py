...
try:
    ...
    for name, df in df_map.items():
        ...
        print(f"\n--- 12. posms: Starptabulu saistību un trūkstošo lauku aizpildīšana tabulai: '{name}' ---")

        if determined_domain == "Veselības aprūpe": # Izpilda tikai veselības aprūpes domēnam
            # Piezīme: Pamatatsauces integritātes pārbaudes jau ir veiktas 9. posmā.
            # Šeit mēs koncentrējamies uz trūkstošo atslēgu aizpildīšanu, izmantojot saistītās tabulas.

            # Pārbaudām tikai tās tabulas, kurām ir PATIENT un ENCOUNTER kolonnas un kuras varētu aizpildīt no encounters
            if name in ["conditions", "allergies", "medications"]:
                if "PATIENT" in df.columns and "ENCOUNTER" in df.columns and "encounters" in df_map:
                    encounters_df = df_map["encounters"].select(
                        col("ID").alias("enc_id"),
                        col("PATIENT").alias("enc_patient_id")
                    )

                    # Filtrējam rindas, kur PATIENT ir NULL un ENCOUNTER nav NULL
                    # Tās ir rindas, kuras mēģināsim aizpildīt
                    df_to_impute = df.filter(col("PATIENT").isNull() & col("ENCOUNTER").isNotNull())

                    if df_to_impute.count() > 0: # Tikai, ja ir rindas, ko aizpildīt
                        # Veicam LEFT OUTER JOIN ar encounters tabulu, lai iegūtu potenciālo PATIENT ID
                        # Ja ENCOUNTER atbilst encounters.ID, mēs iegūstam enc_patient_id.
                        # Ja neatbilst (vai enc_patient_id ir NULL), tad tas būs NULL.
                        imputed_df_temp = df_to_impute.join(
                            encounters_df,
                            lower(df_to_impute.ENCOUNTER) == lower(encounters_df.enc_id),
                            "left_outer"
                        ).withColumn(
                            "IMPUTED_PATIENT_FROM_ENC", # Jauna pagaidu kolonna ar aizpildīto vērtību
                            when(col("enc_patient_id").isNotNull(), col("enc_patient_id")).otherwise(None)
                        )

                        # Saskaitām, cik rindas varētu aizpildīt no encounters tabulas
                        imputed_count = imputed_df_temp.filter(col("IMPUTED_PATIENT_FROM_ENC").isNotNull()).count()

                        if imputed_count > 0:
                            issue_desc = f"Trūkstošo lauku aizpildīšana: Kolonnā 'PATIENT' varētu aizpildīt {imputed_count} trūkstošās vērtības, izmantojot 'ENCOUNTER' saiti uz 'encounters' tabulu."
                            record_issue(results, name, "11FI", "PATIENT_Imputation_from_Encounter", issue_desc, imputed_count, total_rows)
                            print(f"  {issue_desc}")

                            # Ja vēlaties faktiski aizpildīt datus oriģinālajā DataFrame (df_map),
                            # jums būtu jāveic sarežģītāka operācija, lai atjauninātu oriģinālo df.
                            # Piemērs (šis kods nav iekļauts pašreizējā izpildē, bet parāda loģiku):
                            # df_updated = df.join(
                            #     imputed_df_temp.select(col(df_to_impute.ENCOUNTER).alias("original_encounter"), col("IMPUTED_PATIENT_FROM_ENC")),
                            #     lower(df.ENCOUNTER) == lower(col("original_encounter")),
                            #     "left_outer"
                            # ).withColumn(
                            #     "PATIENT",
                            #     coalesce(df.PATIENT, col("IMPUTED_PATIENT_FROM_ENC"))
                            # ).drop("original_encounter", "IMPUTED_PATIENT_FROM_ENC")
                            # df_map[name] = df_updated # Atjaunina DataFrame df_map
                            # print(f"  {imputed_count} 'PATIENT' vērtības tabulā '{name}' tika aizpildītas.")

                        else:
                            print(f"  Trūkstošo lauku aizpildīšana: Netika atrasti aizpildāmi 'PATIENT' lauki, izmantojot 'ENCOUNTER' saiti uz 'encounters' tabulu.")
                            record_issue(results, name, "11FI", "PATIENT_Imputation_from_Encounter", "Netika atrasti aizpildāmi 'PATIENT' lauki", 0, total_rows)
                    else:
                        print(f"  Trūkstošo lauku aizpildīšana: Nav rindu ar trūkstošu 'PATIENT' un esošu 'ENCOUNTER' lauku, ko aizpildīt.")
                        record_issue(results, name, "11FI", "PATIENT_Imputation_from_Encounter", "Nav rindu ar trūkstošu 'PATIENT' lauku, ko aizpildīt", 0, total_rows)
                else:
                    print(f"  Brīdinājums: Nevar veikt 'PATIENT' lauka aizpildīšanu tabulai '{name}', jo trūkst 'PATIENT' vai 'ENCOUNTER' kolonnas, vai 'encounters' tabula.")
                    record_issue(results, name, "11FI", "PATIENT_Imputation_from_Encounter", "Aizpildīšana netika veikta (trūkst atkarību)", 1, 1)
            else:
                record_issue(results, name, "11FI", "Cross_Table_Imputation_Checks", "Starptabulu saistību un trūkstošo lauku aizpildīšanas pārbaudes nav definētas šai tabulai", 0, total_rows)
        else:
            print(f"  Starptabulu saistību un trūkstošo lauku aizpildīšanas pārbaudes netika veiktas, jo domēns nav 'Veselības aprūpe'.")
            record_issue(results, name, "11FI", "Domain_Specific_Cross_Table_Imputation_Checks", "Domēnspecifiskās starptabulu saistību un trūkstošo lauku aizpildīšanas pārbaudes izlaistas (nav 'Veselības aprūpe' domēns)", 0, total_rows)
    ...
    for name, df in df_map.items():
        ...
        # Šis posms ir kritisks, lai risinātu NULL PATIENT ID problēmu, kas tika identificēta analīzē.
        # Mēģinām atrast derīgu PATIENT ID, pat ja tas nav tieši pieejams, izmantojot ENCOUNTER saiti.
        if determined_domain == "Veselības aprūpe":
            if name in reference_rules:
                for fk_col, ref_table_name, pk_col in reference_rules[name]:
                    if ref_table_name in df_map and fk_col in corrected_df.columns and pk_col in df_map[ref_table_name].columns:
                        # Iegūstam atsauces tabulas primārās atslēgas kā sarakstu
                        ref_pk_values = df_map[ref_table_name].select(pk_col).rdd.flatMap(lambda x: x).collect()
                        
                        if not ref_pk_values: # JAUNS: Pārbaude, vai atsauces PK vērtības nav tukšas
                            print(f"  Brīdinājums: Atsauces tabulas '{ref_table_name}' primārās atslēgas kolonna '{pk_col}' ir tukša vai satur tikai NULL vērtības. Atsauces integritātes pārbaude var būt neprecīza.")
                            record_issue(results, name, "15C", f"{fk_col}_Ref_Table_Empty", f"Atsauces tabulas '{ref_table_name}' PK ir tukša. Labošana var būt neprecīza.", 1, corrected_df.count())
                            # Turpinām, bet ņemam vērā, ka labošana var nebūt efektīva
                        
                        ref_pk_broadcast = spark.sparkContext.broadcast(set(ref_pk_values))

                        # "Trīs tabulu variants" - mēģina labot PATIENT ID, ja tas ir nepareizs, bet ENCOUNTER ir pareizs
                        # Šis risina to 2353 ierakstu gadījumu, kurus varēja piesaistīt caur ENCOUNTER.
                        if name in ["conditions", "allergies", "medications"] and fk_col == "PATIENT" and "ENCOUNTER" in corrected_df.columns and "encounters" in df_map and "patients" in df_map:
                            encounters_df_for_join = df_map["encounters"].select(
                                col("ID").alias("enc_id_join"),
                                col("PATIENT").alias("enc_patient_id_derived")
                            )
                            patients_df_for_join = df_map["patients"].select(
                                col("ID").alias("pat_id_join")
                            )

                            # Pievienojam pagaidu kolonnu ar potenciāli laboto PATIENT ID no ENCOUNTER
                            # Pirmkārt, savienojam ar encounters, lai iegūtu enc_patient_id_derived
                            temp_df_for_patient_imputation = corrected_df.alias("current_table") \
                                .join(
                                ...
                                .withColumn(
                                    ...
                                    when(
                                        ...
                                        (lower(col("enc.enc_patient_id_derived")).isin(spark.sparkContext.broadcast(set(patients_df_for_join.select("pat_id_join").rdd.flatMap(lambda x: x).collect())).value)),
                                        col("enc.enc_patient_id_derived")
                                    ).otherwise(None)
                                )
                            
                            # Tagad izmantojam coalesce, lai izvēlētos labāko PATIENT ID:
                            # 1. Oriģinālais PATIENT ID, ja tas ir derīgs (t.i., atrodams `patients` tabulā).
                            # 2. Atvasinātais PATIENT ID no ENCOUNTER, ja tas ir derīgs.
                            # 3. "MISSING_FK" kā galējā vērtība, ja neviens no iepriekšējiem nav derīgs.
                            # Tas palīdzēs identificēt atlikušās neatrisinātās integritātes problēmas.
                            corrected_df = temp_df_for_patient_imputation.withColumn(
                                fk_col,
                                coalesce(
                                    when(
                                        (col(f"current_table.{fk_col}").isNotNull()) & (lower(col(f"current_table.{fk_col}")).isin(ref_pk_broadcast.value)),
                                        col(f"current_table.{fk_col}")
                                    ),
                                    col("derived_patient_id")
                                ).otherwise(lit("MISSING_FK"))
                            ).drop("enc_id_join", "enc_patient_id_derived", "derived_patient_id") # Noņem pagaidu kolonnas
                            
                            print(f"  Kolonnā '{fk_col}': Labotas atsauces integritātes problēmas, izmantojot 'trīs tabulu variantu' uz '{ref_table_name}.{pk_col}'.")
                            record_issue(results, name, "15C", f"{fk_col}_FK_Corrected_Three_Table", f"Atsauces integritātes problēmas labotas ar 'trīs tabulu variantu'.", 0, corrected_df.count())

                        else: # Ja nav "trīs tabulu variants" vai trūkst atkarību, izmantojam tiešo labošanu
                            # Šis solis apstrādā gadījumus, kad nav ENCOUNTER ID vai ENCOUNTER ID nepalīdzēja atrast pacientu.
                            # Šie ieraksti, kas joprojām nav atrisināti, tiek marķēti kā "MISSING_FK".
                            corrected_df = corrected_df.withColumn(
                                fk_col,
                                when(
                                    (col(fk_col).isNotNull()) & (lower(col(fk_col)).isin(ref_pk_broadcast.value)),
                                    col(fk_col)
                                ).otherwise(lit("MISSING_FK"))
                            )
                            print(f"  Kolonnā '{fk_col}': Labotas atsauces integritātes problēmas uz '{ref_table_name}.{pk_col}' (tiešā labošana).")
                            record_issue(results, name, "15C", f"{fk_col}_FK_Corrected_Direct", f"Atsauces integritātes problēmas labotas (tiešā labošana).", 0, corrected_df.count())

                    else:
                        print(f"  Brīdinājums: Nevar labot atsauces integritāti kolonnai '{fk_col}' tabulā '{name}' (trūkst atkarību).")
                        record_issue(results, name, "15C", f"{fk_col}_FK_Correction_Skipped", "Atsauces integritātes labošana izlaista (trūkst atkarību).", 0, corrected_df.count())
            else:
                print(f"  Nav definēti atsauces integritātes noteikumi tabulai '{name}'. Starptabulu labošana izlaista.")
                record_issue(results, name, "15C", "Cross_Table_Correction_Skipped", "Starptabulu labošana izlaista (nav definēti noteikumi).", 0, corrected_df.count())
        else:
            print(f"  Starptabulu integritātes labošana netika veikta, jo domēns nav 'Veselības aprūpe'.")
            record_issue(results, name, "15C", "Domain_Specific_Cross_Table_Correction", "Domēnspecifiskās starptabulu integritātes labošana izlaista (nav 'Veselības aprūpe' domēns)", 0, corrected_df.count())
    ...
    for name, df in corrected_df_map.items():
        ...
        if "START" in df.columns and "STOP" in df.columns:
            wrong_start_stop_pc = df.filter(to_date("STOP") < to_date("START")).count()
                ...
                for fk_col, ref_table_name, pk_col in reference_rules[name]:
                    if ref_table_name in corrected_df_map: # Izmantojam corrected_df_map, lai pārbaudītu pret labotajām tabulām
                        ...
                        if fk_col in df.columns and pk_col in ref_df_pc.columns:
                            # JAUNS: Iegūstam atsauces tabulas primārās atslēgas kā sarakstu no labotajiem datiem
                            ref_pk_values_pc = ref_df_pc.select(pk_col).rdd.flatMap(lambda x: x).collect()
                            
                            if not ref_pk_values_pc: # JAUNS: Pārbaude, vai atsauces PK vērtības nav tukšas
                                ...
                                continue # Pārejam pie nākamās atsauces, jo šī nav uzticama
                            
                            ref_pk_broadcast_pc = spark.sparkContext.broadcast(set(ref_pk_values_pc))

                            # Pārbaudām, vai ir kādas vērtības, kas joprojām nav atsauces tabulā (izņemot "MISSING_FK")
                            invalid_fk_count_pc = df.filter(
                                (col(fk_col).isNotNull()) & 
                                (col(fk_col) != "MISSING_FK") & # Izslēdzam "MISSING_FK" vērtības no pārbaudes
                                (~lower(col(fk_col)).isin(ref_pk_broadcast_pc.value)) # Pārbauda, vai vērtība nav atsauces tabulā
                            ).count()
                            
                            # JAUNS: Saskaitām, cik ierakstu ir ar "MISSING_FK" vērtību
                            missing_fk_count_pc = df.filter(col(fk_col) == "MISSING_FK").count()

                            if invalid_fk_count_pc > 0:
                                issue_desc = f"Atsauces integritātes pārkāpums PĒC-LABOŠANAS: Kolonna '{fk_col}' satur {invalid_fk_count_pc} vērtības, kas neeksistē '{ref_table_name}.{pk_col}' (izņemot 'MISSING_FK')."
                                record_issue(results, name, "17PC_RI", fk_col, issue_desc, invalid_fk_count_pc, total_rows_corrected)
                                # JAUNS: Parāda piemērus neatbilstošajām FK vērtībām
                                print(f"  Piemēri neatbilstošām '{fk_col}' vērtībām (PĒC-LABOŠANAS, izņemot 'MISSING_FK'):")
                                df.filter(
                                    (col(fk_col).isNotNull()) & 
                                    (col(fk_col) != "MISSING_FK") & 
                                    (~lower(col(fk_col)).isin(ref_pk_broadcast_pc.value))
                                ).select(fk_col).limit(5).show(truncate=False)
                            else:
                                record_issue(results, name, "17PC_RI", fk_col, f"Atsauces integritāte OK PĒC-LABOŠANAS uz '{ref_table_name}.{pk_col}' (izņemot 'MISSING_FK').", 0, total_rows_corrected)
                            
                            if missing_fk_count_pc > 0:
                                issue_desc_missing_fk = f"Atsauces integritātes labojums PĒC-LABOŠANAS: Kolonnā '{fk_col}' ir {missing_fk_count_pc} vērtības, kas tika atzīmētas kā 'MISSING_FK'."
                                record_issue(results, name, "17PC_RI", f"{fk_col}_MISSING_FK", issue_desc_missing_fk, missing_fk_count_pc, total_rows_corrected)
                                print(f"  Atsauces integritāte: Atrasti {missing_fk_count_pc} ieraksti ar 'MISSING_FK' vērtību kolonnā '{fk_col}' PĒC-LABOŠANAS.")
    ...
    if not results:
        ...
        quality_dimension_map = {
            ...
            "11FI": "Trūkstošo Lauku Aizpildīšana",
            ...
            "17PC_V": "Derīgums (Pēc Labošanas)", # Jauns
            "17PC_T": "Savlaicīgums (Pēc Labošanas)", # Jauns
            "17PC_RI": "Atsauces Integritāte (Pēc Labošanas)", # Jauns
            "17PC_V": "Apjoma Pārbaudes (Pēc Labošanas)" # Jauns (labots, bija 17PC_V divreiz)
        }
        ...
        if not results_df.empty:
            ...
            issues_by_dimension_summary['quality_dimension'] = issues_by_dimension_summary['quality_dimension'].map(quality_dimension_map)
        ...
        if not results_df.empty:
            ...
            top_columns_with_issues.columns = ['Tabula', 'Kolonna', 'Kļūdu Skaits']
            print("Top 10 kolonnas ar visvairāk problēmām:")
